# WriteSpace — Design System & Visual Standard

**Version:** 1.0.0
**Last Updated:** 2025-07-11
**Applies To:** All UI files in the WriteSpace codebase
**Binding:** This document is the single source of truth for HOW every UI file must look and feel. Every worker reads this before writing markup, styles, or component code. When the LLD or PRD is silent on a visual decision, this file decides.

---

## Table of Contents

1. [Design Direction & Tone](#1-design-direction--tone)
...
10. [Route-Specific Design Recipes](#10-route-specific-design-recipes)
11. [Anti-Slop Manifesto & Banned Defaults](#11-anti-slop-manifesto--banned-defaults)
...
WriteSpace is a fully client-side, single-page blog application — a demonstrable MVP for a B2C content management platform. The visual identity must communicate **editorial warmth, creative confidence, and modern simplicity**. It is a writing tool, not a SaaS dashboard. The design must feel like a premium blogging platform (Medium, Ghost, Substack) rather than an admin panel.

### 1.2 Aesthetic Direction: **Editorial + Soft Luxury**

- **Display face** commands attention on the landing hero and section headers — serif or expressive grotesk that says "words matter here."
- **Body face** is clean, readable, and stays out of the way — a workhorse sans-serif optimized for long-form reading.
- **Palette** leans warm and inviting: deep indigo/violet as the expressive accent (inherited from the PRD's hero gradient specification), paired with a warm stone neutral ramp. No cold grays.
- **Layouts** are asymmetric and editorial — split heroes, bento grids, generous whitespace. Never the centred-three-card cliché.
- **Motion** is restrained and purposeful: subtle reveals on scroll, gentle card lifts on hover, a staged hero entrance. Nothing bounces or jiggles.

### 1.3 Tunable Dials

| Dial | Value | Rationale |
|------|-------|-----------|
| DESIGN_VARIANCE | 6/10 | Bold enough to feel distinctive (editorial hero, asymmetric grids) but not avant-garde — must remain usable for reading and writing. |
| MOTION_INTENSITY | 4/10 | Purposeful motion (hero stagger, card lift, skeleton shimmer) but no parallax, no magnetic buttons, no cursor followers. A writing app should feel calm. |
| VISUAL_DENSITY | 5/10 | Balanced. Blog cards and dashboards carry moderate information density; the landing page and post reader lean toward generous whitespace. |
...
**Chosen pairing: Editorial**

| Role | Font Family | Source | CSS Variable |
|------|-------------|--------|--------------|
| Display / Headings | **Fraunces** (serif, optical sizing) | Google Fonts (`Fraunces:opsz,wght@9..144,100..900`) | `--font-display` |
| Body / UI | **Inter Tight** (sans, tight spacing) | Google Fonts (`Inter+Tight:ital,wght@0,100..900;1,100..900`) | `--font-body` |
| Mono (code, data) | **JetBrains Mono** | Google Fonts (`JetBrains+Mono:wght@100..800`) | `--font-mono` |

**Why Fraunces + Inter Tight:**
- Fraunces brings editorial gravitas — it's a "soft-serif" with variable optical sizing that reads beautifully at display sizes and remains legible at smaller headings. It signals "this is a writing platform."
- Inter Tight is Inter's tighter-spaced sibling — familiar, highly readable, but with a more compact rhythm that pairs well with Fraunces' generous proportions. It avoids the "default Inter" AI-tell.
- JetBrains Mono for code blocks, timestamps, and data tables — adds a technical precision note.

**Implementation in `src/index.css` (Tailwind v4 `@theme` block):**
...
**Rule:** Never use `font-family: Inter, Roboto, Arial, system-ui` as the brand voice. Inter Tight is the body default; Fraunces is the display default.

### 2.2 Typographic Scale

Use these Tailwind utility classes — never arbitrary `text-[17px]` values.

| Role | Tailwind Class | CSS Equivalent | Use |
|------|---------------|----------------|-----|
| Hero Display | `text-6xl md:text-7xl lg:text-8xl` | 3.75rem → 6rem | Landing hero h1 — once per page |
| Page Title | `text-4xl md:text-5xl` | 2.25rem → 3rem | Section headers, page titles |
| Section Heading | `text-2xl md:text-3xl` | 1.5rem → 1.875rem | Card group headers, subsection titles |
| Card Title | `text-xl` | 1.25rem | Blog card titles, stat labels |
| Body Large | `text-lg leading-relaxed` | 1.125rem / 1.625 | Hero subhead, intro paragraphs |
| Body | `text-base leading-relaxed` | 1rem / 1.625 | Default body text, blog content |
| Body Small | `text-sm leading-relaxed` | 0.875rem / 1.625 | Meta, captions, timestamps, helper text |
| Mono | `font-mono text-sm` | 0.875rem | Code, data, prices, UUIDs |

### 2.3 Typography Rules (Binding)

- **Display headings** (`text-6xl` and above): `font-display tracking-tighter leading-[0.95]`. Weight 600–700. Never 800/900 as default — they shout.
- **Body text:** `font-body leading-relaxed` (line-height 1.625). Never `leading-normal` (1.5) on body — it's too tight for reading.
- **Line length:** Body prose must be constrained to `max-w-prose` or `max-w-[65ch]`. Never edge-to-edge paragraphs on desktop.
- **Numbers in data tables / prices / timers:** Always add `tabular-nums` to prevent column jitter.
- **Italic display quotes:** Use Fraunces italic (`italic`) for pull-quotes and editorial lead-ins — it has beautiful italic alternates.
- **Heading hierarchy:** Sequential only — h1 → h2 → h3. Never skip levels.

### 2.4 Typography Anti-Patterns (Banned)

- ❌ Single font for everything (Inter for both headings and body)
- ❌ `font-bold` (700) on body text — use `font-medium` (500) for emphasis
- ❌ `text-center` on paragraphs longer than 2 lines
- ❌ `uppercase tracking-widest` on labels — use `text-sm font-medium tracking-wide` instead
- ❌ Em-dashes in UI microcopy (use en-dash `–` or colon)
...
**Rule:** ONE expressive accent + ONE warm neutral family + small set of semantic tokens. Never invent ad-hoc hex values inside components.

The PRD specifies a hero gradient `from-indigo-600 via-violet-600 to-pink-500`. We adopt **indigo/violet** as our expressive accent family and **stone** as our neutral ramp.

### 3.2 Semantic Colour Tokens

Declare these in `src/index.css` inside the `@theme` block. Reference them in components via Tailwind utility classes — **never use raw hex values in component className strings.**
...
@theme {
  ...
  --color-surface-2: #f5f5f4;    /* stone-100 — elevated surfaces, hover states */
  --color-surface-inverse: #1c1917; /* stone-900 — dark surfaces (footer, nav) */

  /* Text */
  --color-text: #1c1917;          /* stone-900 — body text */
  --color-text-muted: #78716c;    /* stone-500 — secondary text, captions */
  --color-text-inverse: #fafaf9;  /* stone-50 — text on dark surfaces */

  /* Borders & Dividers */
  --color-border: #e7e5e4;        /* stone-200 — card rings, dividers */
  --color-border-hover: #d6d3d1;  /* stone-300 — hover state borders */

  /* Accent (Indigo/Violet family — expressive, not everywhere) */
  --color-accent: #4f46e5;        /* indigo-600 — primary CTA, links, focus rings */
  --color-accent-hover: #4338ca;  /* indigo-700 — button hover */
  --color-accent-light: #e0e7ff;  /* indigo-100 — accent backgrounds, badges */
  ...
  --color-focus: #4f46e5;         /* indigo-600 — focus ring colour */
}
```

### 3.3 Palette Usage Rules

| Element | Token | Tailwind Class Pattern |
|---------|-------|----------------------|
| Page background | `canvas` | `bg-canvas` |
| Card background | `surface` | `bg-surface` |
| Card hover / selected | `surface-2` | `bg-surface-2` |
| Body text | `text` | `text-text` |
| Secondary text | `text-muted` | `text-text-muted` |
| Dividers, card rings | `border` | `ring-border` or `border-border` |
| Primary buttons, links | `accent` | `bg-accent text-accent-fg` |
| Focus rings | `focus` | `ring-focus` |
| Success messages | `success` | `text-success bg-success-light` |
| Error messages | `danger` | `text-danger bg-danger-light` |
| Warning messages | `warning` | `text-warning bg-warning-light` |

### 3.4 Gradient Usage

The PRD specifies a hero gradient: `from-indigo-600 via-violet-600 to-pink-500`. This is the **only approved full-bleed gradient** in the application. It appears on:

1. **Landing page hero** (`/` route) — full-viewport background
2. **Login page** (`/login` route) — full-viewport background
3. **Register page** (`/register` route) — full-viewport background
4. **Admin dashboard banner** (`/admin` route) — contained banner, not full-bleed

**Implementation:**

```html
<!-- Full-viewport gradient (landing, login, register) -->
<div class="min-h-[100svh] bg-gradient-to-br from-indigo-600 via-violet-600 to-pink-500">
  ...
</div>

<!-- Contained gradient banner (admin dashboard) -->
<div class="bg-gradient-to-r from-indigo-600 via-violet-600 to-pink-500 rounded-2xl p-8 md:p-12">
  ...
</div>
```

**Rule:** Do NOT use this gradient anywhere else. Do NOT create new purple→pink gradients. The accent colour (`indigo-600`) is sufficient for buttons, links, and focus rings.

### 3.5 Contrast Requirements (Non-Negotiable)

| Element | Minimum Ratio | Verification |
|---------|--------------|--------------|
| Body text vs background | ≥ 4.5:1 | `stone-900` on `stone-50` = 14.2:1 ✅ |
| Large text (≥18pt or 14pt bold) | ≥ 3:1 | `stone-900` on `stone-50` = 14.2:1 ✅ |
| Icons and disabled text | ≥ 3:1 | `stone-500` on `stone-50` = 4.5:1 ✅ |
| Focus ring vs adjacent surface | ≥ 3:1 | `indigo-600` on `stone-50` = 5.3:1 ✅ |
| White text on accent | ≥ 4.5:1 | `white` on `indigo-600` = 6.2:1 ✅ |

### 3.6 Dark Mode

WriteSpace v1.0 does NOT implement dark mode. All designs target light mode only. If dark mode is added in a future version, it must follow the craft.md rule: desaturate, drop one tonal step on surfaces, keep accent hue but reduce saturation 10-15%. Test contrast independently — dark mode is NOT inverted light mode.

### 3.7 Colour Accessibility
...
All padding, gap, and margin values step in 4px / 8px units. Use Tailwind's spacing scale exclusively — never raw `px` values or arbitrary `gap-[7px]`.
...
| `p-2` / `gap-2` | 8px | Inline meta, icon + text |
| `p-3` / `gap-3` | 12px | Inside compact cards |
| `p-4` / `gap-4` | 16px | Default card padding, form groups |
| `p-6` / `gap-6` | 24px | Between paragraphs, list items, card sections |
| `p-8` / `gap-8` | 32px | Generous card padding, feature sections |
| `py-12` / `gap-12` | 48px | Between top-level sections |
| `py-16 md:py-24` | 64px / 96px | Major section spacing |
| `py-24 md:py-32 lg:py-40` | 96px / 128px / 160px | Hero section — once per page |

### 4.2 Section Spacing Tiers

| Tier | Classes | Use |
|------|---------|-----|
| Tight | `space-y-3` / `gap-3` | Inside cards, inline meta, form field groups |
| Default | `space-y-6` / `gap-6` | Between paragraphs, list items, card grid gaps |
| Section | `space-y-12 py-16 md:py-24` | Between top-level sections on landing, blog list |
| Hero | `py-24 md:py-32 lg:py-40` | Landing hero, login/register pages — only once |

**Rule:** When in doubt, add MORE space. Cramped is the #1 signal of generic AI output.

### 4.3 Container Widths

| Context | Class | Use |
|---------|-------|-----|
| Content pages | `max-w-6xl mx-auto px-4 md:px-6 lg:px-8` | Blog list, post reader, write/edit, user management |
| Dashboard | `max-w-7xl mx-auto px-4 md:px-6 lg:px-8` | Admin dashboard (wider for stat grid) |
| Narrow forms | `max-w-md mx-auto` | Login, register forms inside their gradient wrapper |
| Blog prose | `max-w-prose` or `max-w-[65ch]` | Post content body — never edge-to-edge |

### 4.4 Layout Patterns

**Preferred patterns (use these):**

1. **Editorial split** — Large display text left, body + CTA right, asymmetric column widths (e.g., `grid-cols-12` with `col-span-7` / `col-span-5`). Used on the landing hero.
2. **Bento grid** — Uneven tiles, 2–3 visual weights per row, varies density. Used for the features section and admin dashboard stats. Tailwind `grid-cols-6` with `col-span-{2,3,4}` mixes.
3. **Respons
